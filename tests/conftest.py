...
``os.environ.setdefault`` is intentionally NOT used here because
``pytest-cov`` has already set ``COVERAGE_FILE`` to a tmp dir by the time
this conftest is loaded; setdefault would silently no-op. We force-set
``COVERAGE_PROCESS_START`` and ``COVERAGE_FILE`` so that child Python
interpreters spawned during the test session (subprocess.run, etc.) write
their coverage shards next to the repo root where ``coverage combine``
will find them. See skill leaf
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_web_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
